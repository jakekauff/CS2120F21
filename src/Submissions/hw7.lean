...
begin
  ...
  contradiction,
      ...
      /-
      Given that there exists a set that  
      -/
...
begin
  ...
  refl,
  apply exists.intro p.succ,-- n.succ,
...
begin
  ...
  rw p2,
  rw p1,
  have mulassoc := mul_assoc a2 a1 x,
  rw mulassoc,
end 
...
-- Divides is NOT symmetric, as shown by the cases where 2 divides 4 but
-- 4 does not divide 2. In order for a relation to be symmetric, the relation
