...
variables {α β : Type} -- __Avoid writing ∀ {α β : T} repeatedly__
...
notation a ∈ s := mem a s -- __s applied to a , now lean will turn a ∈ s to mem a s__
...
∀ ⦃a : α⦄, a ∈ s₁ → a ∈ s₂    -- __s1 is a subset of s2 if all values in s1 are in s2__
-- every value in the first set is in the second set
/-
...
notation s₁ ⊆ s₂ := subset s₁ s₂ -- __establishes the notation of subsets__
...
{a | a ∈ s ∧ p a}  -- set of all objects a where a is in s and a satisfies p
...
def even_primes : set ℕ := sep evens primes
...
{b | b = a ∨ b ∈ s} -- new set with a and all from original set
...
def singleton (a : α) : (set α) := 
  {b | b = a} -- the set containing one value a 
...
{a | a ∈ s₁ ∨ a ∈ s₂} -- set of values a such that a is in s1 or a is in s2
...
{a | a ∈ s₁ ∧ a ∈ s₂} -- set of values a such that a is in s1 AND a is in s2
...
{a | a ∉ s}                      -- set of values (of this type) that are not in s
...
{a | a ∈ s ∧ a ∉ t}        -- set of values a such that a is in s and a is not in t
...
{t | t ⊆ s}                              -- set of all sets t such that t is a subset of s
...
{b | ∃ a, a ∈ s ∧ f a = b}                  -- set of b's such that there exists an a in s for which f applied to a equals b
                                            -- image of s under the function f
/-
