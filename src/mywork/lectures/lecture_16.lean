...
the set.                                        -- restricted to type T
                                                OPERATIONS:
For example, consider the set, written in       -- Empty set: {}, nothing in it
what's caledl display notation: { 1, 2, 3, 4 }. -- Complete set: the complement to the empty, has everything in it
The set has four members, as enumerated here.   -- Union → {1,2,3} & {3,4,5} = {1,2,3,4,5} **Sets don't double count
The key distinction we want to make is between  -- Intersection → {1,2,3} & {3,4,5} = {3}
the type of the elements, here ℕ, and the type  -- Complement → All natural evens = All natural odds
of the set, here, set ℕ.                        -- Power set →  A set of all subsets of a given set
                                                      {1,2,3} = {{1},{1,2},{1,3},{2},{3},{2,3},{},{1,2,3}}
                                                      Power set of {1,2,3} is of type "set of set of naturals"
                                                      Size of power set = 2^(size of original set)

                                                -- Subset → A subset K of set S is a set where S contains all 
                                                            objects in set K.

                                                -- Product of two sets → {nice, mean} * {cat, dog} = 
                                                                        {(nice,cat),(mean,cat),(nice,dog),(mean,dog)}
                                                                        The product of two sets contains the product of the 
                                                                        number of elements in one and the number in the other.

                                                                        {Even natural numbers} → infinite, the set of elements n
                                                                        such that n%2 = 0. 
                                                                        {n : ℕ | n % 2 = 0} → defined by a predicate. The predicate
                                                                        is a function that takes a nat and gives a prop. 
...
comes with the corresponding epiphany. 
                                               ...
                                               - We have defined S to be the set of all sets that don't
                                               contain themselves. 
                                               Does S contain itself or not? 
                                               By excluded middle, can only be yes OR no. 
                                               But if S contains itself, then it doesn't contain itself. 
                                               But if S doesn't contain itself, then it would have to contain itself.

                                               Now a set only contains objects of a certain type. The addition
                                               of types provides a layering where the set can't be a member of itself
                                               because a set isn't of that specified type.

                                               Sets != Just a Collection of Things
